...
bool assignTCHF(TransactionEntry& transaction, SDCCHLogicalChannel *SDCCH, TCHFACCHLogicalChannel *TCH)
{
	...
	L3Frame *result = SDCCH->recv(T3107ms+3000);
	if (result==NULL) {
...
LCH->send(L3AuthenticationRequest());
...
LCH->send(L3CipheringModeCommand());
...
LOG(INFO)<< "Ciphering Completed ";
     ...
     LOG(INFO) <<  "Authentication Reject ";
		...
		LCH->send(L3CMServiceReject(0x21));
	LOG(INFO) << "CM Service Reject ";
        LCH->send(L3ChannelRelease(0x6f));
...
//	LCH->send(L3CMServiceAccept());
	...
	LOG(INFO) << *setup << "SETUP CHECK POINT";
...
L3AuthenticationResponse *response = dynamic_cast<L3AuthenticationResponse*>(msg);
  if (!response) {
   if (msg) {
    LOG(WARN) << "Unexpected message " << *msg;
    delete msg;
...
LOG(INFO)<< "Ciphering Completed";
     ...
     LOG(INFO) <<  "Authentication Reject";
        ...
        LCH->send(L3ChannelRelease());
	LOG(INFO) << "Channel Release";
        LCH->send(L3CMServiceReject(0x06));
	LOG(INFO) << "CM Service Reject";
        return;
}
