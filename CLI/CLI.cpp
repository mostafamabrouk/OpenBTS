...
{
	if (argc!=2) {
		os << "usage: testA3 ... shouf fe 3'alta fel parameters <imsiprefix>\n";
		return BAD_VALUE;
	}
	...
	unsigned int SRES;
	SRES=a3a8.getSRES();
	LOG(DEBUG) << "KcAmaNshouf = " << Kc;
	os << "kc=" << Kc << endl;
	LOG(DEBUG) << "SRESAmaNshouf = " << SRES;
	os << "SRES=" << SRES << endl;


	uint32_t frameno;
	frameno=gKiTable.getFrameNumber();
	LOG(DEBUG) << "frameNumberYaRab = " << frameno;

	A51 a51;
	a51.keysetup(Kc,frameno);
	int ks[114];
	for (int i = 0; i < 114; ++i) 
	     ks[i] = a51.generate_keystream_bit();
/*	  
	for (int i = 0; i < 114; ++i) 
		os << "ks=" << int(ks[i]) << " -" << i << endl;

	os << "ks=" << ks << endl;
*/
    }
	else {
		os << " shouf fe 3'alta fel gsmConfig ... rou7 e3mlo copy men el backup " << endl;
		return BAD_VALUE;
	}
	return SUCCESS;
}
