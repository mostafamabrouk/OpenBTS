...
{
	LOG(ERROR) << "not implemented for " << MTI();
	assert(0);
}
...
L3RRMessage* GSM::L3RRFactory(L3RRMessage::MessageType MTI)
{
	switch (MTI) {
		...
		// Don't even parse this.  We will ignore unparsed messages in the control layer.
...
size_t L3CipheringModeComplete::bodyLength() const
{
	size_t sum = 0;
/*
