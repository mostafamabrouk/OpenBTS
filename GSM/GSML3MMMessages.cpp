...
ostream& GSM::operator<<(ostream& os, L3MMMessage::MessageType val)
{
	switch (val) {
        ...
        os << "AuthenticationRequest"; break;
        case L3MMMessage::AuthenticationResponse:
        os << "AuthenticationResponse"; break;
...
L3MMMessage* GSM::L3MMFactory(L3MMMessage::MessageType MTI)
{
   ...
   case L3MMMessage::AuthenticationRequest: return new L3AuthenticationRequest;
...
{
    dest.writeField(wp,0,4);		// spare half octet
	dest.writeField(wp,0,4);		//mCKSN.writeV(dest,wp);
...
/*size_t L3AuthenticationResponse::bodyLength() const
{
 size_t len = 0;
 len += mSRES.lengthV();
 return len;
...
void L3AuthenticationReject::writeBody(L3Frame& dest, size_t &wp ) const
{
...
void L3AuthenticationReject::text(ostream& os) const
{
 L3MMMessage::text(os);
}

/*
size_t L3AuthenticationReject::bodyLength() const
{
size_t len = 0;
